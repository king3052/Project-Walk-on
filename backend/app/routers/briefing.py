...
def get_morning_briefing(current_user_id: str = Depends(check_ai_rate_limit), db: Session = Depends(get_db)):
    ...
    prompt = (
        ...
        "trend, today's schedule, and active goals below, write a short (2-4 sentence) briefing that "
        "names the ONE highest-impact thing to focus on today. If readiness is low, factor that into "
        "the recommendation (suggest lighter work, not just 'push harder'). Be specific and reference "
        "the actual numbers given — never invent a number that isn't in the data below. If there isn't "
